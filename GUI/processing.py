...
def go(timeout_seconds=60, log_callback=None): # Added a timeout parameter (e.g., 60 seconds)
    ...
    while not is_cry_detected():
        ...
        if elapsed_time > timeout_seconds :
            if log_callback:
                log_callback(f"⏱️ Timeout reached ({timeout_seconds}s) with no audio detected. Exiting loop.")
