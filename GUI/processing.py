...
def process_and_save_audio(output_filename="after noise removal.wav", duration=10, sr=16000,log_callback=None):
    ...
    # 2. Noise Floor Guard: Ignore frame if it's just room silence
    if max_val < 0.01:
        return False
        
    # 3. Controlled Peak Normalization (scales peak to 80% maximum, avoiding hard clipping)
    raw_audio = raw_audio / max_val * 0.8
    ...
    vad = wvad.Vad(1)  # Change 0 to 3 in both functions
...
'''def is_cry_detected(duration=0.03, sr=16000, threshold=0.03):
    raw_audio = sd.rec(int(duration * sr), samplerate=sr, channels=1, dtype='float32')
    sd.wait()
    raw_audio = np.squeeze(raw_audio)
    
    # Calculate the volume/energy of the sound
